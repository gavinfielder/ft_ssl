...
/*   Updated: 2019/05/31 21:06:59 by gfielder         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			parse_error(const char *str, t_queue *tasks)
{
	...
	if (str)
		ft_dprintf(2, "%s\n", str);
	ft_dprintf(2, "usage: ./ft_ssl sha256 [-pqr] [-s string] [files ...]\n");
	exit(-1);
}

static void			parse_single(t_ssl *opt, char c)
{
	static char		buff[38] = "./ft_ssl sha256: unknown option -- c";
	char			*task;
...
static t_ssl		parse_arguments(t_ssl *opt, int count, char **args)
{
	...
	while (++i < count)
	{
		...
		{
			if (i + 1 >= count)
				parse_error("./ft_ssl sha256: option requires an argument -- s",
						opt->tasks);
...
t_ssl				sha256_parse_command(int argc, char **argv)
{
	...
	if (!(opt.tasks = ft_queue_new(FTQ_AUTO_STR)))
		parse_error("./ft_ssl sha256: task queue creation failure.", opt.tasks);
	i = 0;
