...
/*   Updated: 2019/05/27 18:07:45 by gfielder         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	md5_read_helper(t_block *blk, int64_t *len,
				int64_t ret, int64_t bufflen)
{
	...
	if (bufflen < 56)
	{
		...
		int64_copy(((unsigned char *)blk) + 56, *len);
...
int			md5_read(int fd, void *blk_v, int64_t *len,
				int prev_status)
{
	int64_t	ret;
	int64_t	bufflen;
	...
	bufflen += ret;
	return (md5_read_helper(blk, len, ret, bufflen));
}
