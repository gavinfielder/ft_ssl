...
/*   Updated: 2019/05/27 18:07:52 by gfielder         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int64_t	md5_string_read_init(char **msg, char *raw, int64_t *len,
					int64_t *next)
{
	size_t	alen;
	...
	*msg = ft_strnew(alen);
	ft_strcpy(*msg, raw);
	(*msg)[*len] = (char)0x80;
	int64_copy(((unsigned char *)(*msg)) + alen - 8, *len);
	*next = 0;
	return (alen);
}

int				md5_string_read(int action, void *ptr, int64_t *len,
				int prev_status)
{
	...
	{
		return ((total_size = md5_string_read_init(&msg,
						(char *)ptr, len, &next)));
	}
